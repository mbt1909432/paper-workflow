...
\begin{enumerate}[leftmargin=*]
    \item We propose the \textbf{first framework} that embeds multi-dimensional trustworthiness constraints into every stage of the GraphRAG pipeline, from retrieval decision to output verification. The framework defines trust dimensions (factuality, robustness, fairness, transparency, accountability, and privacy) that guide each module's design.
