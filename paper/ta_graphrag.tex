...
\begin{enumerate}[leftmargin=*]
    \item We propose the \textbf{first framework} that embeds six-dimensional trustworthiness constraints (factuality, robustness, fairness, transparency, accountability, and privacy) into every stage of the GraphRAG pipeline, from retrieval decision to output verification.
    ...
    \item We develop a \textbf{three-stage hallucination detection module} that leverages graph path reasoning for causal verification, achieving more precise fact-checking than purely text-based approaches.
...
\end{tikzpicture}
\caption{The overall architecture of TA-GraphRAG. The system integrates six-dimensional trustworthiness constraints into every stage of the GraphRAG pipeline, with an iterative refinement loop triggered when the overall trust score falls below threshold $\tau$.}
\label{fig:framework}
\end{figure*}
...
\textbf{Datasets.} We evaluate TA-GraphRAG on the following benchmarks:
\begin{itemize}[leftmargin=*]
    \item \textbf{HotpotQA}~\cite{yang2018hotpotqa}: Multi-hop question answering requiring reasoning across multiple documents.
    \item \textbf{2WikiMultiHopQA}~\cite{ho20202wiki}: Complex multi-hop reasoning over Wikipedia.
    \item \textbf{MuSiQue}~\cite{trivedi2022musique}: Compositional multi-hop QA with careful construction.
    \item \textbf{TRC Bench}~\cite{zhou2024trustworthiness}: RAG trustworthiness benchmark covering six trust dimensions.
    \item \textbf{TruthfulQA}~\cite{lin2022truthfulqa}: Measuring model truthfulness.
\end{itemize}
...
\textbf{(3)} TA-GraphRAG demonstrates the highest internal quality awareness: the trust filter consistently assigns high trust scores (mean 0.87) to retained documents, and the hallucination detection module identifies low-confidence answers with mean hallucination scores of 0.30. This self-assessment capability is a key differentiator, even though it does not directly translate to higher EM/F1 in this pilot.
...
To understand the contribution of each module, we analyze the internal metrics produced by TA-GraphRAG's components across the 10 test questions:
